...
id=tag.data('id');location.href='detail.html?id='+id;},initList:function(query){$.ajax({type:'GET',url:'http://news-at.zhihu.com/api/'+apiVersion+'/section/'+query,data:{},dataType:'json',timeout:15000,success:function(res){var
...
tpl='',tmpImg,i;for(i=0;i<rs.length;i++){tmpImg=rs[i].images?rs[i].images[0]:'';tpl+='<li class="bottom-li J_clickEvent" data-event="gotoEvent" data-id="'+rs[i].id+'">';tpl+='<p>';tpl+='<span>'+rs[i].title+'</span><br>';tpl+='<span>'+rs[i].display_date+'</span>';tpl+='</p>';tpl+='<img src="'+tmpImg+'">';tpl+='</li>';}
