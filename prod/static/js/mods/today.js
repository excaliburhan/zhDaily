...
date=res.date,stories=res.stories,topStories=res.top_stories;$('.loading').remove();page.renderList(date,topStories,stories);},error:function(){$('.loading').remove();$('body').html('<div class="error">接口异常</div>');}});},renderList:function(date,topStories,stories){var
fDate=page.formatDate(date),fts=page.formatTop(topStories),fs=page.formatStories(stories);$('.date').html(fDate);$('.top-stories').html(fts);$('.stories').html(fs);page.initSlider();},formatDate:function(date){var
